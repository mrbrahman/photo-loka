...
export async function updateGeoAddress(uuid, geo_address, status = 'FOUND_DB_EXACT_MATCH', matched_uuid = null) {
  return await asyncRun(`UPDATE metadata SET geo_address = ?, geo_status = ?, geo_matched_uuid = ? WHERE uuid = ?`, geo_address, status, matched_uuid, uuid);
}

export async function updateGeonamesData(uuid, geonames_json, geo_address) {
  return await asyncRun(`UPDATE metadata SET geonames_json = ?, geo_address = ?, geo_status = ? WHERE uuid = ?`, geonames_json, geo_address, 'FOUND_VIA_API', uuid);
}
